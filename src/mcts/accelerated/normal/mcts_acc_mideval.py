...
def mid_eval(chessboard):
	return chessboard[0, 0] + chessboard[0, -1] + chessboard[-1, 0] + chessboard[-1, -1] + 0.5
...
def rollout(chessboard, color):
	if np.sum(chessboard != COLOR_NONE) >=20:
		con = 0
	else:
		con = 1
	while check_end(chessboard) == False:
		...
		if len(candidate_list) == 0:
			...
			color = -color
	return con * judge(chessboard) + (1-con) * mid_eval(chessboard)
