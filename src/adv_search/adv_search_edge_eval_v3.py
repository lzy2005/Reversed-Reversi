...
def eval(chessboard):
	...
	edge_val = 0
	if chessboard[0, 0] == 0:
		edge_val += -(chessboard[1, 0] + chessboard[0, 1])
	if chessboard[0, -1] == 0:
		edge_val += -(chessboard[0, -2] + chessboard[1, -1])
	if chessboard[-1, -1] == 0:
		edge_val += -(chessboard[-1, -2] + chessboard[-2, -1])
	if chessboard[-1, 0] == 0:
		edge_val += -(chessboard[-2, 0] + chessboard[-1, 1])
	star_val = (chessboard[1, 1] + chessboard[1, -2] + chessboard[-2, 1] + chessboard[-2, -2])

	if depth >= 16:
		return corner_val * 50 + edge_val * 4 + sum_val
	elif depth >= 8:
		return corner_val * 30 + edge_val * 3 + sum_val
	elif depth >= 4:
		return corner_val * 20 + edge_val * 3 + sum_val
	else:
		return corner_val * 8 + edge_val * 2 + sum_val
