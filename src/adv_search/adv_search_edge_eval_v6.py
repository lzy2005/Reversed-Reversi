...
def eval(chessboard):
	sum_black = (chessboard == -1).sum()
	sum_white = (chessboard == 1).sum()
	depth = GRID_NUM - (sum_black + sum_white)

	sum_val = sum_white - sum_black
	corner_val = chessboard[0, 0] + chessboard[0, -1] + chessboard[-1, 0] + chessboard[-1, -1]

	triv_edge_val, spec_edge_val = 0, 0
	if chessboard[0, 0] == 0:
		a, b, c = chessboard[0, 1], chessboard[1, 0], chessboard[1, 1]
		if a == b and b == c and c != 0 and (c == chessboard[0, 2] or c == chessboard[2, 0]):
			spec_edge_val += -a
		else:
			triv_edge_val += -(a + b)
	if chessboard[0, -1] == 0:
		a, b, c = chessboard[0, -2], chessboard[1, -1], chessboard[1, -2]
		if a == b and b == c and c != 0 and (c == chessboard[0, -3] or c == chessboard[2, -1]):
			spec_edge_val += -a
		else:
			triv_edge_val += -(a + b)
	if chessboard[-1, -1] == 0:
		a, b, c = chessboard[-1, -2], chessboard[-2, -1], chessboard[-2, -2]
		if a == b and b == c and c != 0  and (c == chessboard[-3, -1] or c == chessboard[-1, -3]):
			spec_edge_val += -a
		else:
			triv_edge_val += -(a + b)
	if chessboard[-1, 0] == 0:
		a, b, c = chessboard[-2, 0], chessboard[-1, 1], chessboard[-2, 1]
		if a == b and b == c and c != 0  and (c == chessboard[-3, 0] or c == chessboard[-1, 2]):
			spec_edge_val += -a
		else:
			triv_edge_val += -(a + b)

	if depth >= 16:
		return corner_val * 30 + spec_edge_val * 20 + triv_edge_val * 4 + sum_val
	elif depth >= 4:
		return corner_val * 20 + spec_edge_val * 15 + triv_edge_val * 3 + sum_val
	else:
		return corner_val * 8 + spec_edge_val * 6 + triv_edge_val * 2 + sum_val
	# spec +
