...
spec = [
	...
	('adv_board', numba.types.Array(numba.int8, 3, 'C')),
	('adv_cnt', numba.int32)
]
...
class MCT(object):
	def __init__(self, chessboard_size, color, time_out, start_time, chessboard):
		...
		self.adv_board = np.zeros((SIZE_ADV, self.chessboard_size, self.chessboard_size), dtype=np.int8)
		self.adv_cnt = 0
...
def adv_search(mct, idx, color, alpha, beta):
	chessboard = mct.adv_board[idx]
	if check_end(chessboard):
		return judge(chessboard)

	candidate_list = get_candidate_list(chessboard, color)
	if len(candidate_list) == 0:
		return adv_search(mct, idx, -color, alpha, beta)
	if color == COLOR_BLACK:
		value = -math.inf
		for pos in candidate_list:
			mct.adv_cnt += 1
			mct.adv_board[mct.adv_cnt] = chessboard # ok?
			get_next_board(mct.adv_board[mct.adv_cnt], pos, color)
			result = adv_search(mct, mct.adv_cnt, -color, alpha, beta)
...
@numba.jit(numba.float32(
	...
	numba.types.Array(numba.int8, 2, 'C'),
	numba.int8), nopython=True)
def adv_judge(mct, chessboard, color):
	mct.adv_cnt = 0
	mct.adv_board[mct.adv_cnt] = chessboard #ok?
	return adv_search(mct, mct.adv_cnt, color, -math.inf, math.inf)
...
@numba.jit(numba.float32(
	...
	numba.int8,
numba.int8), nopython=True)
def rollout(mct, chessboard, color, depth):
	while check_end(chessboard) == False and depth < ADV_DEPTH:
		...
		if len(candidate_list) == 0:
			...
			color = -color
			depth += 1
	return adv_judge(mct, chessboard, color)
...
def aug(mct):
	...
	if check_end(mct.state[node]) or (node != mct.root and mct.dep[node] >= ADV_DEPTH):
		if mct.ans[node] == -1:
			mct.ans[node] = adv_judge(mct, mct.state[node], mct.color[node])
		backward(mct, node, mct.ans[node])
		return
	elif mct.n[node] == 0:
		backward(mct, node, rollout(mct, mct.state[node].copy(), mct.color[node], mct.dep[node]))
	else:
		if mct.last > SIZE:
			backward(mct, node, rollout(mct, mct.state[node].copy(), mct.color[node], mct.dep[node]))
			return
