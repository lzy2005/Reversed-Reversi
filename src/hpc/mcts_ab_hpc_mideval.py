...
@numba.jit(numba.float32(numba.types.Array(numba.int8, 2, 'C')), nopython=True)
def mid_eval(chessboard):
	return (chessboard[0, 0] + chessboard[-1, -1] + chessboard[0, -1] + chessboard[ -1, 0])/8 + 0.5
...
def rollout(chessboard, color, depth):
	...
	while check_end(chessboard) == False and depth < ADV_DEPTH:
		...
		if len(candidate_list) == 0:
			...
			depth += 1
			con *= GAMA
	return con * adv_search(chessboard, color, -math.inf, math.inf) + (1 - con) * res
